module "euc1-vpc-complete" {
  for_each = { for k, v in try(var.aws_resources[local.euc1].vpc, {}) : k => v }
...
module "euc1-log_groups" {
  for_each          = local.euc1_log_groups
...
module "euc1-alb" {
  for_each = { for k, v in try(var.aws_resources[local.euc1].alb, {}) : k => v }
  source   = "./modules/alb"
  ...
  vpc_id     = module.euc1-vpc-complete[each.value.vpc].vpc_id
  subnet_ids = module.euc1-vpc-complete[each.value.vpc].public_subnets
  # certificate_arn = module.euc1-acm[each.value.certificate_domain_name].arn

  depends_on = [module.euc1-vpc-complete]
  tags       = local.tags
  providers = {
    aws = aws.euc1
  }
...
module "euc1-ecs-cluster" {
  for_each = { for k, v in try(var.aws_resources[local.euc1].ecs_cluster, {}) : k => v }
...
module "euc1-ecs-service" {
  for_each = { for k, v in try(var.aws_resources[local.euc1].ecs_service, {}) : k => v }
  source   = "./modules/ecs-service"

  name               = each.key
  vpc_id             = module.euc1-vpc-complete[each.value.vpc].vpc_id
  alb_security_group = module.euc1-alb[each.value.alb].alb_security_group_id
  alb_target_group   = module.euc1-alb[each.value.alb].alb_target_group_arn[each.value.alb_target]
  ecs_cluster_arn    = module.euc1-ecs-cluster[each.value.ecs_cluster].ecs_cluster_arn
  ...
  depends_on = [module.euc1-ecs-cluster, module.euc1-alb, module.euc1-vpc-complete, module.euc1-log_groups]
